...
/**
 * Quét mã vạch bằng camera — dùng chung cho NHIỀU ô, không riêng "Mã chuyến đi".
 *
 * Ô đích khai bằng thuộc tính trên nút bấm:
 *   <button data-scan="maChuyenDi">                → quét xong ĐIỀN ĐÈ, đóng luôn
 *   <button data-scan="md-0" data-scan-mode="append"> → quét xong THÊM vào danh
 *        sách ngăn bởi dấu phẩy, KHÔNG đóng — quét tiếp mã kiện thứ 2, 3…
 *
 * Nút của ô "Mã đơn liên quan" được sinh động lúc dựng danh sách hàng hoá, nên
 * bắt sự kiện theo kiểu uỷ quyền ở document — gắn trực tiếp sẽ hụt nút sinh sau.
 *
...
function initScan() {
  if (!scanSupported()) {
    ...
    show($('scanHint'), true);
    // Nút sinh sau (ô mã đơn) cũng phải khoá — xử lý trong renderHangHoa.
    return;
  }
